...
- **上游账号状态异常**：账号凭证过期或被禁用
- **通道处于冷却期**：近期该通道请求失败，系统自动冷却（软失败固定 10 秒，硬失败按 fibonacci 退避 60s×fib(n)，round_robin 路由走分级冷却 10m→60m→24h）。可在路由管理页面点击「❄ 解除冷却」手动恢复
- **站点处于惩罚/熔断中**：连续错误触发站点级惩罚或熔断。可在路由管理页面点击「↻ 撤销处罚」手动恢复
- **上游模型下线**：上游站点已移除该模型
- **余额不足**：对应账号余额已耗尽
