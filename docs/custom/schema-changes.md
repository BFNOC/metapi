...
| `sites` | `endpoint_overrides` | `text` | `NULL` | JSON 数组，站点级允许的上游端点协议白名单：`chat` / `messages` / `responses` |
...
### `sites.endpoint_overrides`

JSON 字符串，格式为 `["chat"]`、`["messages", "responses"]` 等。

- `NULL` / 空数组：不覆盖，沿用现有自动协议候选推断
- 非空：作为站点级硬过滤层，只允许请求尝试列出的上游端点协议

该字段只做“过滤已有候选”，不会凭空新增候选，也不会绕过文件能力、平台限制等硬约束。
...
| `src/server/index.ts` → `ensureSiteCompatibilityColumns()` | 运行时入口，复用 `siteSchemaCompatibility.ts` 统一执行 compat |
...
| `drizzle/0021_endpoint_overrides.sql` | `sites.endpoint_overrides` 列 |
...
1. **对应的 compat spec**（如 `siteSchemaCompatibility.ts` / `accountTokenSchemaCompatibility.ts`）— 定义多方言迁移 SQL
2. **对应的 Drizzle migration** — 保证 `drizzle/*.sql` 与 schema contract 构建链一致
3. **generated schema artifacts** — 通过 `npm run schema:contract` 更新 `schemaContract.json` 与 checked-in dialect SQL
...
- `src/server/index.ts` — 运行时 schema compat 调用入口
- `scripts/dev/generate-schema-contract.ts` — 写入 schema contract 与 checked-in dialect artifacts
