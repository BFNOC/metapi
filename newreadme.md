...
| **模型探活增强** | 支持单通道/批量探活、批次间隔配置、反风控优化（随机真实 prompt）、route 级一键应用探活排序；明确故障沉底，健康通道按 TTFT 分档加权，`跳过/未知` 保留当前优先级 |
...
- 管理页会派生展示为四类健康结果：`成功 / 失败 / 跳过 / 未知`
- `401/403/429`、`5xx`、timeout、`fetch failed`、网关/过载/配置型上游错误会被视为**失败**，在一键排序时沉底
- “无有效探活令牌”等前置条件不足会显示为**跳过**，不会再和“未知”混在一起
- 路由卡片摘要、通道 badge 和应用排序使用同一套派生规则，避免统计和单行展示口径分叉
- “应用探活排序”要求基于当前启用通道的完整结果集执行，避免用过期探活快照改写优先级
