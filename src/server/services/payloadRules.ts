...
function getPayloadRulesSectionValue(
  value: Record<string, unknown>,
  key: keyof typeof PAYLOAD_RULE_SECTION_LABELS,
): unknown {
...
function hasConflictingPayloadRuleSectionKeys(
  value: Record<string, unknown>,
  camelKey: 'defaultRaw' | 'overrideRaw',
  dashedKey: 'default-raw' | 'override-raw',
): boolean {
  return Object.prototype.hasOwnProperty.call(value, camelKey)
    && Object.prototype.hasOwnProperty.call(value, dashedKey);
}
...
function validatePayloadValueRuleSectionInput(value: unknown, section: string, raw: boolean): string | null {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    for (const [path, rawValue] of Object.entries(item.params)) {
      ...
      if (!raw || typeof rawValue !== 'string') continue;
...
export function parsePayloadRulesConfigInput(value: unknown): PayloadRulesParseResult {
  ...
  const validationError = validatePayloadValueRuleSectionInput(
    value.default,
    PAYLOAD_RULE_SECTION_LABELS.default,
    false,
  ) ?? validatePayloadValueRuleSectionInput(
    getPayloadRulesSectionValue(value, 'defaultRaw'),
    PAYLOAD_RULE_SECTION_LABELS.defaultRaw,
    true,
  ) ?? validatePayloadValueRuleSectionInput(
    value.override,
    PAYLOAD_RULE_SECTION_LABELS.override,
    false,
  ) ?? validatePayloadValueRuleSectionInput(
    getPayloadRulesSectionValue(value, 'overrideRaw'),
    PAYLOAD_RULE_SECTION_LABELS.overrideRaw,
    true,
  ) ?? validatePayloadFilterRuleSectionInput(
    value.filter,
    PAYLOAD_RULE_SECTION_LABELS.filter,
  );
