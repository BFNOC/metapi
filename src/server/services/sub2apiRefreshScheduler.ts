...
function normalizedLifecycleStatusSql(
  column: typeof schema.accounts.status | typeof schema.sites.status,
) {
...
export async function executeSub2ApiManagedRefreshPass(input: {
  ...
  const workerCount = Math.max(
    1,
    Math.min(SUB2API_REFRESH_SCHEDULER_CONCURRENCY, refreshCandidates.length || 1),
  );
...
async function runScheduledSub2ApiRefreshPass(): Promise<void> {
  ...
  sub2ApiRefreshPassInFlight = executeSub2ApiManagedRefreshPass()
    ...
    .catch((error) => {
      console.warn(
        `[sub2api-refresh] scheduled pass failed: ${(error as Error)?.message || 'unknown error'}`,
      );
    })
    .finally(() => {
...
export function startSub2ApiManagedRefreshScheduler(
  intervalMs = SUB2API_REFRESH_SCHEDULER_INTERVAL_MS,
) {
  clearSub2ApiRefreshSchedulerTimer();

  const safeIntervalMs = Math.max(
    SUB2API_REFRESH_SCHEDULER_INTERVAL_MS,
    Math.trunc(intervalMs || 0),
  );
