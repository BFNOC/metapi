...
describe('ensureSiteSchemaCompatibility', () => {
  it.each([
    {
      ...
      expectedSql: [
        ...
        'ALTER TABLE "sites" ADD COLUMN "endpoint_overrides" TEXT',
        ...
        'UPDATE "sites" SET "model_filter_mode" = \'deny-list\' WHERE "model_filter_mode" IS NULL',
        'ALTER TABLE "sites" ADD COLUMN "probe_disabled" BOOLEAN DEFAULT FALSE',
        'UPDATE "sites" SET "probe_disabled" = FALSE WHERE "probe_disabled" IS NULL',
        ...
        'CREATE TABLE IF NOT EXISTS "site_allowed_models" ("id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "site_id" INTEGER NOT NULL REFERENCES "sites"("id") ON DELETE CASCADE, "model_name" TEXT NOT NULL, "created_at" TEXT)',
    ...
    },
    {
      ...
      expectedSql: [
        ...
        'ALTER TABLE `sites` ADD COLUMN `endpoint_overrides` TEXT NULL',
        'ALTER TABLE `sites` ADD COLUMN `external_checkin_url` TEXT NULL',
        'ALTER TABLE `sites` ADD COLUMN `global_weight` DOUBLE DEFAULT 1',
        'UPDATE `sites` SET `global_weight` = 1 WHERE `global_weight` IS NULL OR `global_weight` <= 0',
        'ALTER TABLE `sites` ADD COLUMN `model_filter_mode` VARCHAR(20) DEFAULT \'deny-list\'',
        'UPDATE `sites` SET `model_filter_mode` = \'deny-list\' WHERE `model_filter_mode` IS NULL',
        'ALTER TABLE `sites` ADD COLUMN `probe_disabled` BOOLEAN DEFAULT FALSE',
        'UPDATE `sites` SET `probe_disabled` = FALSE WHERE `probe_disabled` IS NULL',
        ...
        'CREATE TABLE IF NOT EXISTS `site_allowed_models` (`id` INT AUTO_INCREMENT PRIMARY KEY, `site_id` INT NOT NULL, `model_name` VARCHAR(191) NOT NULL, `created_at` TEXT NULL, CONSTRAINT `site_allowed_models_site_fk` FOREIGN KEY (`site_id`) REFERENCES `sites`(`id`) ON DELETE CASCADE)',
  ...
  ])('adds missing site compatibility schema objects for $dialect', async ({ dialect, expectedSql }) => {
