...
describe('sub2apiRefreshScheduler', () => {
  ...
  it('refreshes due sub2api accounts with bounded concurrency instead of serially', async () => {
    ...
    expect(refreshSub2ApiManagedSessionSingleflightMock).toHaveBeenCalledTimes(
      sub2ApiRefreshSchedulerConcurrency,
    );
  ...
  it('runs one scheduled pass at a time and waits for in-flight work on stop', async () => {
    vi.useRealTimers();
    const nowMs = Date.parse('2026-04-06T02:00:00.000Z');
    vi.setSystemTime(nowMs);

    const site = await db.insert(schema.sites).values({
      name: 'sub2-scheduled-site',
      url: 'https://sub2-scheduled.example.com',
      platform: 'sub2api',
      status: 'active',
    }).returning().get();
    ...
    await db.insert(schema.accounts).values({
      ...
      accessToken: 'scheduled-access-token',
      apiToken: null,
      status: 'active',
      extraConfig: buildSub2ApiExtraConfig({
    ...
    const intervalHandle = startSub2ApiManagedRefreshScheduler();
    expect(intervalHandle).toEqual({
      enabled: true,
      intervalMs: 60_000,
    });
