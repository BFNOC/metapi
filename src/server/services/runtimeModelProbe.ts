...
function resolveRemainingTimeoutMs(deadlineAtMs: number, timeoutLabel: string): number {
  ...
  if (remainingMs <= 0) throw new Error(timeoutLabel);
  return remainingMs;
}
...
export async function probeRuntimeModel(input: {
  ...
  if (!isLikelyConversationModel(input.modelName)) {
    return { status: 'skipped', latencyMs: null, reason: 'skipped non-conversation model probe' };
  }
  ...
  if (!tokenValue) {
    return { status: 'inconclusive', latencyMs: null, reason: 'missing credential for probe' };
  }
  ...
  try {
    const endpointCandidates = await withTimeout(
      () => resolveUpstreamEndpointCandidates(
        { site: input.site, account: input.account },
        input.modelName,
        'openai',
        input.modelName,
      ),
      resolveRemainingTimeoutMs(deadlineAtMs, `runtime model probe candidate resolution timeout (${Math.round(input.timeoutMs / 1000)}s)`),
      `runtime model probe candidate resolution timeout (${Math.round(input.timeoutMs / 1000)}s)`,
    );
    if (endpointCandidates.length <= 0) {
      return { status: 'inconclusive', latencyMs: Date.now() - startedAt, reason: 'no compatible probe endpoint candidates' };
    }

    const providerHeaders = buildOauthProviderHeaders({ account: input.account, downstreamHeaders: {} });
    ...
    const remainingMs = resolveRemainingTimeoutMs(deadlineAtMs, `runtime model probe timeout (${Math.round(input.timeoutMs / 1000)}s)`);
    ...
    const dispatchRequest = async (request: BuiltEndpointRequest, targetUrl: string) => (
    ...
    if (result.ok) {
      ...
      return { status: 'supported', latencyMs, reason: 'probe succeeded' };
    }
