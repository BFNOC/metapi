...
describe('POST /api/routes/:id/cooldown/clear', () => {
  ...
  const seedAccountWithToken = async () => {
    ...
    const account = await db.insert(schema.accounts).values({
      ...
      apiToken: `cooldown-api-token-${id}`,
      status: 'active',
    }).returning().get();
  ...
  it('clears cooldown fields for a direct route while preserving failCount', async () => {
  ...
  it('only clears cooldown for visible explicit-group source routes and preserves failCount', async () => {
    const visibleSeed = await seedAccountWithToken();
    const disabledSeed = await seedAccountWithToken();
    const wildcardSeed = await seedAccountWithToken();
    ...
    const visibleChannel = await db.insert(schema.routeChannels).values({
      ...
      accountId: visibleSeed.account.id,
      tokenId: visibleSeed.token.id,
      sourceModel: 'gpt-5.4',
      priority: 0,
      weight: 10,
      enabled: true,
    ...
    const disabledChannel = await db.insert(schema.routeChannels).values({
      ...
      accountId: disabledSeed.account.id,
      tokenId: disabledSeed.token.id,
      sourceModel: 'gpt-4.1',
      priority: 0,
      weight: 10,
      enabled: true,
    ...
    const wildcardChannel = await db.insert(schema.routeChannels).values({
      ...
      accountId: wildcardSeed.account.id,
      tokenId: wildcardSeed.token.id,
    ...
    expect(refreshedDisabled).toMatchObject({
      failCount: 6,
      lastFailAt: '2026-04-01T00:00:00.000Z',
      consecutiveFailCount: 2,
      cooldownLevel: 3,
    ...
    expect(refreshedWildcard).toMatchObject({
      failCount: 7,
      lastFailAt: '2026-04-01T00:00:00.000Z',
      consecutiveFailCount: 2,
      cooldownLevel: 3,
