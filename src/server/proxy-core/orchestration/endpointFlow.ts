...
export async function executeEndpointFlow(input: ExecuteEndpointFlowInput): Promise<EndpointFlowResult> {
  ...
  if (endpointCount <= 0) {
    return { ok: false, status: 502, errText: 'Upstream request failed' };
  }
  ...
  for (let endpointIndex = 0; endpointIndex < endpointCount; endpointIndex += 1) {
    ...
    let response = await fetchWithObservedFirstByte(
      ...
      { firstByteTimeoutMs: input.firstByteTimeoutMs, startedAtMs: attemptStartedAtMs },
    );

    if (response.ok) {
      await runEndpointFlowHook(input.onAttemptSuccess, {
        endpointIndex, endpointCount, request, targetUrl, response, recoverApplied: false,
      }, 'onAttemptSuccess');
      return { ok: true, upstream: response, upstreamPath: request.path };
    }

    let rawErrText = await readRuntimeResponseText(response).catch(() => 'unknown error');
    const baseContext: EndpointAttemptContext = {
      endpointIndex, endpointCount, request, targetUrl, response, rawErrText, recoverApplied: false,
    };
    ...
    if (isObservedFirstByteTimeoutResponse(response) && !isLastEndpoint) {
      ...
      await runEndpointFlowHook(input.onAttemptFailure, { ...baseContext, errText }, 'onAttemptFailure');
      finalStatus = response.status || 408;
      finalErrText = errText;
      finalRawErrText = rawErrText;
      if (input.disableCrossProtocolFallback) break;
      continue;
    }
    ...
    if (input.tryRecover) {
      ...
      if (recovered?.upstream?.ok) {
        ...
        await runEndpointFlowHook(input.onAttemptSuccess, {
          endpointIndex, endpointCount, request: recoveredRequest,
          targetUrl: recoveredTargetUrl, response: recovered.upstream, recoverApplied: true,
        }, 'onAttemptSuccess');
        return { ok: true, upstream: recovered.upstream, upstreamPath: recovered.upstreamPath };
      }
    }

    rawErrText = baseContext.rawErrText;
    response = baseContext.response;
    const errText = withUpstreamPath(baseContext.request.path, summarizeUpstreamError(response.status, rawErrText));
    await runEndpointFlowHook(input.onAttemptFailure, { ...baseContext, errText }, 'onAttemptFailure');
    ...
    const shouldAbort = !isLastEndpoint && !!input.shouldAbortRemainingEndpoints?.({ ...baseContext, errText });
    if (shouldAbort) {
      finalStatus = response.status;
      finalErrText = errText;
      finalRawErrText = rawErrText;
    ...
    if (shouldDowngrade) {
      await runEndpointFlowHook(input.onDowngrade, { ...baseContext, errText }, 'onDowngrade');
      continue;
    }
  ...
  return { ok: false, status: finalStatus || 502, errText: finalErrText || 'unknown error', rawErrText: finalRawErrText };
}
