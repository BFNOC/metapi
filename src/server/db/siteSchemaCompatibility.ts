...
export const SITE_COLUMN_COMPATIBILITY_SPECS: SiteColumnCompatibilitySpec[] = [
  ...
  },
  {
    ...
    addSql: {
      sqlite: "ALTER TABLE sites ADD COLUMN model_filter_mode text DEFAULT 'deny-list';",
      mysql: "ALTER TABLE `sites` ADD COLUMN `model_filter_mode` VARCHAR(20) DEFAULT 'deny-list'",
      postgres: "ALTER TABLE \"sites\" ADD COLUMN \"model_filter_mode\" TEXT DEFAULT 'deny-list'",
    },
    normalizeSql: {
      sqlite: "UPDATE sites SET model_filter_mode = 'deny-list' WHERE model_filter_mode IS NULL;",
      mysql: "UPDATE `sites` SET `model_filter_mode` = 'deny-list' WHERE `model_filter_mode` IS NULL",
...
export const SITE_TABLE_COMPATIBILITY_SPECS: SiteTableCompatibilitySpec[] = [
  ...
  },
  {
    ...
    createSql: {
      sqlite: 'CREATE TABLE IF NOT EXISTS site_allowed_models (id integer PRIMARY KEY AUTOINCREMENT NOT NULL, site_id integer NOT NULL REFERENCES sites(id) ON DELETE cascade, model_name text NOT NULL, created_at text DEFAULT (datetime(\'now\')));',
      mysql: 'CREATE TABLE IF NOT EXISTS `site_allowed_models` (`id` INT AUTO_INCREMENT PRIMARY KEY, `site_id` INT NOT NULL, `model_name` VARCHAR(191) NOT NULL, `created_at` TEXT NULL, CONSTRAINT `site_allowed_models_site_fk` FOREIGN KEY (`site_id`) REFERENCES `sites`(`id`) ON DELETE CASCADE)',
      postgres: 'CREATE TABLE IF NOT EXISTS "site_allowed_models" ("id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "site_id" INTEGER NOT NULL REFERENCES "sites"("id") ON DELETE CASCADE, "model_name" TEXT NOT NULL, "created_at" TEXT)',
    },
    postCreateSql: {
      sqlite: [
        'CREATE UNIQUE INDEX IF NOT EXISTS site_allowed_models_site_model_unique ON site_allowed_models (site_id, model_name);',
        'CREATE INDEX IF NOT EXISTS site_allowed_models_site_id_idx ON site_allowed_models (site_id);',
      ],
      mysql: [
