...
| `no available channel` | 路由无可用通道 | 检查 Token 是否同步、通道是否被冷却；可在路由页查看冷却状态，或点击「❄ 解除冷却」手动恢复 |
| `channel cooling` | 通道进入冷却期 | 软失败（流中断等）固定冷却 10 秒；硬失败按 fibonacci 退避（60s × fib(n)）；round_robin 路由走分级冷却（10m → 60m → 24h）；也可在路由页手动解除 |
...
路由引擎会根据错误类型对通道和站点施加不同程度的冷却和惩罚：

| 错误类型 | 冷却策略 | 说明 |
|----------|---------|------|
| 软完成失败（流中断等） | 固定 10 秒 | 短暂避让，不升级 |
| 硬失败（5xx/超时/鉴权等） | fibonacci 退避（60s × fib(n)） | 连续失败时冷却时间递增 |
| 硬失败（round_robin 路由） | 分级冷却（10m → 60m → 24h） | 每 3 次连续失败升级一级 |
| 上游 429 限流 | 触发站点熔断 | 3 分钟 → 15 分钟 → 60 分钟分级 |
| 客户端取消 | 不冷却 | 不是站点的错 |

当通道或站点陷入异常冷却时，可在 **路由管理页面** 手动干预：

| 按钮 | 位置 | 操作 |
|------|------|------|
| **↻ 撤销处罚**（橙色药丸） | 通道行内，站点有惩罚时显示 | 清除站点运行时惩罚 + 所有通道 DB 冷却状态 |
| **❄ 解除冷却**（蓝色药丸） | 通道行内，通道冷却中时显示 | 仅清除单个通道的冷却状态 |

> 健康模型的完整设计文档见 [routing-health-model.md](./engineering/routing-health-model.md)。
